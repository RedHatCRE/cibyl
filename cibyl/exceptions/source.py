...
class NoSupportedSourcesFound(CibylException):
    ...
    def __init__(self, system: str, function: str) -> None:
...
class NoValidSources(CibylException):
    ...
    def __init__(self, system: str = "", sources: str = "") -> None:
        if sources:
            sources = Colors.blue('\n  '.join(sources))
            sources = f"\nAvailable sources:\n  {sources}"
        if system:
...
class MissingArgument(SourceException):
    ...
    def __init__(self, message: str = 'Missing required argument.') -> None:
        """Constructor.
        """
...
class InvalidArgument(SourceException):
    ...
    def __init__(self, message: str = 'Invalid argument passed.') -> None:
        """Constructor.
        """
        super().__init__(message)
