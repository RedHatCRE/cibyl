...
IP_VERSION = "--network-protocol\s+ipv[4|6]"
IP_VERSION_NUMBER = "ipv4|ipv6"
    ...
    def _get_ip_version(self, path, **kwargs):
        ...
        if "ip_version" in kwargs:
            ...
            for script in root.iter("script"):
                ...
                for line in lines:
                    ip_version_lst = re.find(IP_VERSION, line)
                    for el in ip_version_lst:
                        nodeSet = set(re.findall(IP_VERSION_NUMBER, el))
                        result = result.union(nodeSet)
            ip_version_str = ",".join(result)
            # filtering support e.g. --ip-version ipv4
            if kwargs['ip_version'].value is not None and len(
                    list(filter(lambda x: kwargs['ip_version'].value[0] in x,
                                result))) == 0:
                return None
